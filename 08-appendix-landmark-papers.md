...
> Scope: ten foundational works the modern story rests on — each with a summary and why it mattered. PDFs are downloaded locally into `papers/` for offline study; that folder is **not** committed to the public repo (the files are copyrighted), so each entry also links to an online **source**.
...
*Generated as part of the Foundations of Modern AI study series. Source of truth: this `.md` file; the `.html` is built from it via `./build.sh`. PDFs are the property of their respective authors/publishers; they are kept locally for study and are not redistributed in this repository — use the per-entry source links.*
